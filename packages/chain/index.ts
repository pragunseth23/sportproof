...
export function createMarketClient(config:ChainConfig) {
 ...
 async function confirmReceipt(hash:Hash){await assertReady();const receipt=await publicClient.getTransactionReceipt({hash});if(receipt.status!=='success')throw new Error('Transaction reverted');let canonical,latest;for(let attempt=0;;attempt++){try{[canonical,latest]=await Promise.all([publicClient.getBlock({blockNumber:receipt.blockNumber}),publicClient.getBlock({blockTag:'latest'})]);break}catch(lagError){if(attempt>=3)throw lagError;await new Promise(r=>setTimeout(r,2500))}}if(canonical.hash!==receipt.blockHash)throw new Error('Receipt was reorganized');if(latest.number-receipt.blockNumber+1n<BigInt(confirmations))throw new Error('Awaiting sealed L2 confirmations');return receipt;}
